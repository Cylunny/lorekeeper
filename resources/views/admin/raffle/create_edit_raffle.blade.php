...
<h1>{{ $raffle->id ? 'Edit' : 'Create' }} Raffle
    @if($raffle->id)
    <a href="#" class="btn btn-danger float-right delete-raffle-button">Delete Raffle</a>
    @endif
</h1>
...
<div class="text-right">
    {!! Form::submit( $raffle->id ? 'Edit' : 'Create', ['class' => 'btn btn-primary']) !!}
</div>
{!! Form::close() !!}
...
$( document ).ready(function() {    
    $('.delete-raffle-button').on('click', function(e) {
        e.preventDefault();
        loadModal("{{ url('admin/raffles/delete') }}/{{ $raffle->id }}", 'Delete Raffle');
    });
});
</script>
@endsection