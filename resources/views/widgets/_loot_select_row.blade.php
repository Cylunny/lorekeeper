<div id="lootRowData" class="hide">
    <table class="table table-sm">
        <tbody id="lootRow">
            <tr class="loot-row">
                <td>{!! Form::select('rewardable_type[]', ['Item' => 'Item', 'Currency' => 'Currency'] + ($showLootTables ? ['LootTable' => 'Loot Table'] : []) + ($showRaffles ? ['Raffle' => 'Raffle Ticket'] : []) + (isset($characters) ? ['Character' => 'Character'] : []), null, ['class' => 'form-control reward-type', 'placeholder' => 'Select Reward Type']) !!}</td>
    ...
    @endif
    @if(isset($characters))
        {!! Form::select('rewardable_id[]', $characters, null, ['class' => 'form-control character-select', 'placeholder' => 'Select Character']) !!}
    @endif
</div>