    ...
    <tbody id="lootTableBody">
        @if($loots)
            @foreach($loots as $loot)
                <tr class="loot-row">
                    <td>{!! Form::select('rewardable_type[]', ['Item' => 'Item', 'Currency' => 'Currency'] + ($showLootTables ? ['LootTable' => 'Loot Table'] : []) + ($showRaffles ? ['Raffle' => 'Raffle Ticket'] : []) + (isset($characters) ? ['Character' => 'Character'] : []), $loot->rewardable_type, ['class' => 'form-control reward-type', 'placeholder' => 'Select Reward Type']) !!}</td>
                    <td class="loot-row-select">
                        ...
                        @elseif($loot->rewardable_type == 'Character' && isset($characters))
                            {!! Form::select('rewardable_id[]', $characters, $loot->rewardable_id, ['class' => 'form-control character-select selectize', 'placeholder' => 'Select Character']) !!}
                        @endif
                    </td>
